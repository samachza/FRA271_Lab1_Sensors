...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(Encoder_reader_simulink_M, 0);
  ...
  Encoder_reader_simulink_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Encoder_reader_simulink_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Encoder_reader_simulink_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Encoder_reader_simulink_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
  ...
  /* Terminate model */
  Encoder_reader_simulink_terminate();
